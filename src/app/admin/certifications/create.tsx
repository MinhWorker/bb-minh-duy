  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Create failed"); // Re-throw with the server's message
      }

      notify("Certification created successfully!")
  ...
  return (
    <Create>
      <SimpleForm onSubmit={handleSubmit}>
        <TextInput source="name" label="Certification Name" fullWidth required />
        <ImageInput
        ...
        <p className="text-red-500 text-sm">*Support up to 10mb</p>
      </SimpleForm>
    </Create >
  )
}
